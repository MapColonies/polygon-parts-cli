...
export class CSVToDB {
  ...
  public async csvToPg(): Promise<ProcessingSummary> {
    ...
    if (!this.validateHeaders(mappedKeys))
      throw new Error("Headers validation error");
      ...
      try {
        ...
        for await (row of readStream) {
          ...
          for (const polygon of polygons) {
            polygonCounter += 1;
            console.log(
              `Processing row number ${rowNumber} -- polygon ${polygonCounter} out of ${polygons.length}`,
            );
        ...
        console.log(
          "Calculating polygon parts, this might take a while for large inserts 🤌",
        );
  ...
  private processRow(
    ...
    const minHorizontalAccuracyCE90 = getValue(
      row,
      mappedKeys.minHorizontalAccuracyCE90,
    );
    const maxHorizontalAccuracyCE90 = getValue(
      row,
      mappedKeys.maxHorizontalAccuracyCE90 ??
        mappedKeys.minHorizontalAccuracyCE90,
    );
    ...
    const polygonPartRecord: PartRecord = {
      ...
      sourceDateStartUTC: getValue(
        row,
        mappedKeys.sourceDateStartUTC ?? mappedKeys.sourceDateEndUTC,
      ),
      sourceDateEndUTC: getValue(row, mappedKeys.sourceDateEndUTC),
      minResolutionDegree: parseFloat(
        getValue(
          row,
          mappedKeys.minResolutionDegree ?? mappedKeys.maxResolutionDegree,
        ),
      ),
      maxResolutionDegree: parseFloat(
        getValue(row, mappedKeys.maxResolutionDegree),
      ),
      minResolutionMeter: parseFloat(
        getValue(
          row,
          mappedKeys.minResolutionMeter ?? mappedKeys.maxResolutionMeter,
        ),
      ),
      maxResolutionMeter: parseFloat(
        getValue(row, mappedKeys.maxResolutionMeter),
      ),
      minHorizontalAccuracyCE90:
        minHorizontalAccuracyCE90 === null
          ? minHorizontalAccuracyCE90
          : parseFloat(minHorizontalAccuracyCE90),
      maxHorizontalAccuracyCE90:
        maxHorizontalAccuracyCE90 === null
          ? maxHorizontalAccuracyCE90
          : parseFloat(maxHorizontalAccuracyCE90),
    ...
    Object.entries(polygonPartRecord).forEach(([key, value]) => {
      if (isInArray(key, OPTIONAL_FIELDS) && value === "")
        polygonPartRecord[key] = null;
    });
          ...
          for (let i = 0; i < row.length; i++) {
            ...
            if (isInArray(key, ALL_FIELDS)) {
              ...
              throw new CSVHeaderValidationError(
                key,
                VALIDATION_ERRORS.unsupportedHeader,
              );
          }
        })
        .on("end", () => {
  ...
  private validateHeaders(
    headers: Partial<FieldsMapping>,
  ): headers is FieldsMapping {
    const keys = Object.keys(headers);

    const missingHeaders = REQUIRED_FIELDS.filter(
      (requiredField) => !keys.includes(requiredField),
    );
    if (missingHeaders.length > 0)
      throw new Error(
        `Failed validation of headers -- missing headers: ${missingHeaders}`,
      );
  ...
  private validateRow(
    ...
    for (const field of REQUIRED_FIELDS) {
      if (!row[mappedKeys[field]])
        throw new CSVContentValidationError(
          field,
          VALIDATION_ERRORS.mandatoryField,
          rowNumber,
          undefined,
        );
    }
    ...
    if (!SUPPORTED_GEO_TYPES.includes(geoType))
      throw new CSVContentValidationError(
        "geom",
        VALIDATION_ERRORS.geometryType,
        rowNumber,
        undefined,
      );
  }
  ...
  private mergeOverlappingMultiParts(polygons: Polygon[]): Polygon[] {
    ...
    for (let i = 1; i < overlappingPartsIndices.length - 1; i++) {
      overlappingParts =
        union(overlappingParts, polygons[overlappingPartsIndices[i]]) ??
        overlappingParts;
    }

    const overlappingPolygonParts = flatten(overlappingParts).features.map(
      (feature) => feature.geometry,
    );
    const nonOverlappingPolygonParts = polygons.filter(
      (_, overlappingPartIndex) =>
        !overlappingPartsIndices.includes(overlappingPartIndex),
    );

    return [...overlappingPolygonParts, ...nonOverlappingPolygonParts];
  }
  ...
  private getPolygons(wkt: string, rowNumber: number): Polygon[] {
    ...
    switch (geoJson.type) {
      case "Polygon":
        return [geoJson];
      case "MultiPolygon":
        const polygonParts = geoJson.coordinates.map<Polygon>(
          (polygonCoordinates) => {
      ...
      default:
        throw new Error(
          "Only MULTIPOLYGON and POLYGON geometry types are supported",
        );
    }
  }
}

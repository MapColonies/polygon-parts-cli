...
class FileToDB {
    ...
    return new Promise((resolve, reject) => {
      fs.createReadStream(this.inputPath)
        ...
        .on("data", async (row) => {
          ...
          // pass validation for current row in csv and will process data to db for current row
  ...
  validateData = (row) => {
    ...
    if (!row[this.columnMappedKeys["wkt"]]) {
      ...
      const wkt = row[this.columnMappedKeys["wkt"]];
      const geoType = wkt.split(" ")[0];
      if (geoType !== "POLYGON" && geoType !== "MULTIPOLYGON") {
        isValid = false;
        console.error(
          `wkt should be consist of Polygon or MultiPolygon only types, provided wkt: "${geoType}"`
        );
      }
    }
